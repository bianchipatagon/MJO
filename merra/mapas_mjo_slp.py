...
# ~ colores = ['mediumblue' if v <= 0 else 'red' for v in ds1[field1].data]
niveles = [-2.5,-2, -1.5,-1,-0.5, 0,0.5,1,1.5,2,2.5]

colores = ['red', 'red', 'red', 'red', 'red','mediumblue','mediumblue','mediumblue','mediumblue']
...
for k, ax in enumerate(axes):
  ...
  # ~ im = ax.pcolormesh(ds1['lon'].data, ds1['lat'].data, ds1[field1].data*0.01, shading='nearest', cmap = 'RdYlBu_r', vmin=-2.5, vmax=2.5)
  # ~ im = ax.pcolormesh(ds1['lon'].data, ds1['lat'].data, ds1[field1].data*0.01, shading='nearest', cmap = 'RdYlBu_r')
  ax.contour(ds1['lon'].data, ds1['lat'].data, ds1[field1].data*0.01,levels=niveles, colors=colores, alpha = 0.8, linewidths= 1.2)
...
# ~ cb.set_label('slp [hpa]', fontsize = 12)
...
plt.savefig('slp.png',bbox_inches="tight", dpi=600)
